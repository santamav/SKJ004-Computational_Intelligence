...
globals [ payoff_matrix ]
players-own [ strategy cumulative_profit opponent ]
...
to go
  ask players [
    clear-opponents ;; Clear the opponent from the previous loop
    find-opponent ;; Find a new opponent
    update-player-profit ;; Update first the profit for all the turtles
  ]
...
to setup-globals
  set payoff_matrix [[2 0] [3 1]] ;; Payof matrix values
end

to setup-players
  ;; Create the cooperative players
  create-players cooperating_players [
    setxy random-xcor random-ycor ;; Random position
    set strategy "C" ;; Random strategy
    update-player-color ;; Update color based on the strategy
    set cumulative_profit 0 ;; Set initial profit
    set opponent nobody
  ]
  ;; Create the defects
    ;; Create the cooperative players
  create-players (participants - cooperating_players) [
    setxy random-xcor random-ycor ;; Random position
    set strategy "D" ;; Random strategy
    update-player-color ;; Update color based on the strategy
    set cumulative_profit 0 ;; Set initial profit
    set opponent nobody
  ]
...
to find-opponent
  if opponent = nobody [
    let other_player one-of players with [opponent = nobody]
    ask opponent [set opponent myself]
    set opponent other_player
  ]
end

;; Update player profit
to update-player-profit
  ;; Get the strategy indexes for each player
  let opponent_strategy [strategy] of opponent
...
to update-player-strategy
  ...
  if (rdn < strategy_change_prob) [
    let the_other one-of players
    ;; Select the best strategy
    set strategy ifelse-value (cumulative_profit < [cumulative_profit] of the_other)[[strategy] of the_other][strategy]
    update-player-color]
end
...
to-report get-profit [p_strategy o_strategy]
  let p_strategy_value strategy-value p_strategy
  let opponent_strategy_value strategy-value o_strategy
