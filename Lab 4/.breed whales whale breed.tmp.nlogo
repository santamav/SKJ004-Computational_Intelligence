...
to receive-response-to-cfps
  foreach initiator-cfps [ _cfps -> ;; _cfps = [conversation_id, [whale, ticks+]]
    ...
    let lmsgs get-lmsgs-key mailbox "conversation_id" _id;; Get all the answers to the current cfps conversation
    if not empty? lmsgs[
      set mailbox remove-from-lmgs mailbox lmsgs ;; Remove read messages from the mailbox
      let proposals get-lmsgs-key lmsgs "performative" "proposal" ;; Get all the proposal answers
      ifelse not empty? proposals [
...
to response-accept-proposal [winner _id _whale]
  ;; Prepare the message with all its attributes
  let msg create-message "accept-proposal"
  set msg set-value-msg msg "conversation_id" _id
  set msg set-value-msg msg "content" _whale
  ;; Send message to winner
  send-message msg get-value-msg winner "sender"
end

;; Send the reject proposal message
to response-reject-proposal [lmsgs _id]
  ;; Prepare the message
  let msg create-message "reject-proposal"
  set msg set-value-msg msg "conversation_id" _id
  ;; Send message to the boats
  foreach lmsgs [
    _msg ->
    send-message _msg get-value-msg _msg "sender"
  ]
end

;;; For the rol of follower
to respond-new-cfps
  let new_cfps get-lmsgs-key mailbox "performative" "cfps" ;; Get all the new cfps messages

  if not empty? new_cfps [
    set mailbox remove-from-lmgs mailbox new_cfps ;; Remove messages from the mailbox
  ]

  foreach new_cfps [ _msg ->
    let _id get-value-msg _msg "conversation_id" ;; Get the conversation id
    let _x  get-value-msg _msg "content" ;; Get the whales data
    let _sender get-value-msg _msg "sender" ;; Get the cps sender to respond to
...
to respond-proposal [_sender _id _distance]
  ;; Create message and set attributes
  let msg create-message "proposal"
  set msg set-value-msg msg "conversation_id" _id
  set msg set-value-msg msg "content" _distance
  ;; Send message to sender
  send-message msg _sender
  ;; Add message to follower-cfps
  set follower-cfps lput _id follower-cfps
end
...
to receive-responses-to-proposals-cfps
  ...
  foreach follower-cfps [
    _id ->
           let lmsgs get-lmsgs-key mailbox "conversation_id" _id
           if not empty? lmsgs [
...
to-report get-min-msg [lmsgs]
  let values map [_msg -> get-value-msg _msg "content"] lmsgs ;; Get the values from the messages
  let min_value min values ;; Get the smaller value
  report filter [_msg -> (get-value-msg _msg "content") = min_value] lmsgs ;; Report the msg with the smaller value
end
